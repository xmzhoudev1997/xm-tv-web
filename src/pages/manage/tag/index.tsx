...
import { Button, Dialog, Input, List, NavBar, PullToRefresh } from 'antd-mobile'; import {
  DragDropContext,
  Draggable,
  Droppable,
                  ...
                  {list.map((item, index: number) => (
                    <Draggable key={item.id} draggableId={String(item.id)} index={index}>
                      {(provided, snapshot) => (
                        ...
                        >
                          <List.Item extra={`共${item.subCount}个标签`}>{item.name}</List.Item>
                        </div>
                      )}
                    </Draggable>
                  ))}
    ...
    <div className={styles.root}>
      ...
      >
        <List>
          {
            list.map((item) => (
              <List.Item arrow extra={`共${item.subCount}个标签`} onClick={() => handleAction(item)} key={item.id}>{item.name}</List.Item>
            ))
          }
        </List>
      </PullToRefresh>
